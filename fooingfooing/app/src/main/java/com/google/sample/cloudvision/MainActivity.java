...
public class MainActivity extends AppCompatActivity {
    ...
    public class RecyclerAdapter extends RecyclerView.Adapter<RecyclerAdapter.ViewHolder>{
        ...
        public void onBindViewHolder(ViewHolder holder, int position) {
            String categoryTemp = mainLists.get(position).mCategory;
            String nameTemp = mainLists.get(position).mName;
            ...
            if(!result.get(position).equals("정의를 찾지 못했습니다."))
            {
                holder.tvCategory.setText(categoryTemp);
                holder.tvName.setText(nameTemp);
            }
        ...
        protected String doInBackground(String... strings) {
            ...
            for(int i=0; i<item.size();i++) {
                ...
                try {
                    ...
                    String res = jsonElement.getAsJsonObject().get("return_object").getAsJsonObject().get("WiKiInfo").getAsJsonObject().get("AnswerInfo").getAsJsonArray().get(0).getAsJsonObject().get("answer").toString();
                    res = res.replaceAll("\"","");
                    result.add(res);
            ...
            for(int i=0; i<testArray.length; i++) {
                ...
                try {
                    ...
                    if(!check.equals("[]"))
                    {
                        ...
                        {
                            String itemTemp = jsonElement.getAsJsonObject().get("return_object").getAsJsonObject().get("sentence").getAsJsonArray().get(0).getAsJsonObject().get("NE").getAsJsonArray().get(0).getAsJsonObject().get("text").toString();
                            if(itemTemp.equals("\"소듐\""))
                                 itemTemp="나트륨";

                            if(itemTemp.equals("\"총탄수화물\""))
                                itemTemp="탄수화물";


                            Log.i("gurcks",itemTemp);
                            item.add(itemTemp);
                            typeitem.add(type);
                            mainLists.add(new MainList(itemTemp,type));
                        }
